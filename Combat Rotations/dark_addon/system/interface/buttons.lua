...
fontObject:SetFont("Interface\\Addons\\dark_addon\\media\\OpenSans-Regular.ttf", button_size / 4)

fontObject = CreateFont("dark_addon_small")
fontObject:SetFont("Interface\\Addons\\dark_addon\\media\\OpenSans-Regular.ttf", 12)

fontObject = CreateFont("dark_addon_bold")
fontObject:SetFont("Interface\\Addons\\dark_addon\\media\\OpenSans-Bold.ttf", button_size / 4)

fontObject = CreateFont("dark_addon_icon")
fontObject:SetFont("Interface\\Addons\\dark_addon\\media\\FontAwesomeProRegular.otf", button_size / 2)
...
do
  local minR, minG, minB = dark_addon.interface.color.hexToRgb(dark_addon.interface.color.grey)
...
info_frame.text:SetFontObject("dark_addon_regular")
...
info_frame.text_right:SetFontObject("dark_addon_regular")
info_frame.text_right:SetText('')

dark_addon.interface.status_text_override = false
function dark_addon.interface.status_override(text, duration, colorA, colorB)
  dark_addon.interface.status_text_override = true
  info_frame.text:SetText(text)
  if not colorA or not colorB then
    colorA = dark_addon.interface.color.teal
    colorB = dark_addon.interface.color.dark_teal
  end
  do
  ...
  C_Timer.After(duration, function()
    ...
    dark_addon.interface.status_text_override = false
    info_frame.text:SetText(dark_addon.interface.status_text)
  end)
end

dark_addon.interface.status = function(text)
  dark_addon.interface.status_text = text
  if not dark_addon.interface.status_text_override then
    info_frame.text:SetText(text)
  end
end
  ...
  dark_addon.interface.buttons.add({
    ...
    toggle_on = function(self)
      ...
      if button.label then
        dark_addon.interface.status_override(button.label .. ' enabled', 1)
      end
    end,
    ...
    toggle_off = function(self)
      ...
      if button.label then
        dark_addon.interface.status_override(button.label .. ' disabled', 1)
      end
    end,
...
dark_addon.on_ready(function()
  ...
  dark_addon.interface.buttons.add_toggle({
    ...
    font = 'dark_addon_icon',
    on = {
      label = dark_addon.interface.icon('clock'),
  ...
  dark_addon.interface.buttons.add_toggle({
    ...
    off = {
      label = dark_addon.interface.icon('user'),
      color = dark_addon.interface.color.grey,
      color2 = dark_addon.interface.color.dark_grey
    }
  })
